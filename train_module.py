...
class Trainer(object):
    ...
    def __init__(self, opt):
        ...
        self.optimizer1 = optim.Adam(filter(lambda p: p.requires_grad, self.generator.parameters()), lr=opt.lr,
                                     betas=(0.5, 0.99))
        self.optimizer2 = optim.Adam(filter(lambda p: p.requires_grad, self.discriminator.parameters()), lr=opt.lr,
                                     betas=(0.5, 0.99))
    ...
    def forward(self, input_data, gt, is_train=True, is_test=False):
        ...
        if is_train:
            ...
            return generator_loss, discriminator_loss, error, PSNR, SSIM, map_loss, attention_loss, multi_scale_loss, perceptual_loss, \
            attention_map[-1]
        elif is_test:
    ...
    def train(self):
        for epoch in range(self.start, self.iter_num):
            ...
            for i, data in enumerate(self.train_loader):
                generator_loss, discriminator_loss, error, PSNR, SSIM, map_loss, attention_loss, multi_scale_loss, perceptual_loss, attention_map = self.forward(
                ...
                if (i + 1) % 50 == 0:
                    ...
                    print('train_loader_len : {}'.format(train_loader_len))
            ...
            print('epoch : {}'.format(epoch + 1) + '\n# train dataset error : {}'.format(
                train_eval['error'] / train_loader_len))
            ...
            for i, data in enumerate(self.dev_loader):
                ...
                if i < 10:
                    for j in range(output.shape[0]):
                        cv2.imwrite('./dev_output_{}/{}_rain_{}.png'.format(self.opt.save, 2 * i + j, epoch + 1),
                                    self.get_output_img(data[0][j]))
                        cv2.imwrite('./dev_output_{}/{}_gt_{}.png'.format(self.opt.save, 2 * i + j, epoch + 1),
                                    self.get_output_img(data[1][j]))
                        cv2.imwrite('./dev_output_{}/{}_predict_{}.png'.format(self.opt.save, 2 * i + j, epoch + 1),
                                    self.get_output_img(output[j]))

            print('epoch : {}'.format(epoch + 1) + '\n# development dataset error : {}'.format(
                dev_eval['error'] / dev_loader_len))
        ...
        for i, data in enumerate(test_loader):
            ...
            if i < 20:
                for j in range(output.shape[0]):
                    ...
                    cv2.imwrite('./output_{}/{}_rain.png'.format(self.opt.save, 2 * i + j),
                                self.get_output_img(data[0][j]))
                    cv2.imwrite('./output_{}/{}_gt.png'.format(self.opt.save, 2 * i + j),
                                self.get_output_img(data[1][j]))
                    cv2.imwrite('./output_{}/{}_predict.png'.format(self.opt.save, 2 * i + j),
                                self.get_output_img(output[j]))
